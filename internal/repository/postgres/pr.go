...
func NewPRRepository(pool *pgxpool.Pool, logger *log.Logger) repo.PRRepository {
	return &PRRepository{pool: pool, sb: sq.StatementBuilder.PlaceholderFormat(sq.Dollar), log: logger}
}

func (r *PRRepository) Create(ctx context.Context, pr *entity.PR) error {
	q := r.sb.Insert("prs").Columns("id", "title", "author_id", "status", "created_at", "updated_at").Values(pr.ID, pr.Title, pr.AuthorID, pr.Status, pr.CreatedAt, pr.UpdatedAt)
	sql, args, _ := q.ToSql()
...
func (r *PRRepository) GetByID(ctx context.Context, id string) (*entity.PR, error) {
	q := r.sb.Select("id", "title", "author_id", "status", "created_at", "updated_at").From("prs").Where(sq.Eq{"id": id})
	sql, args, _ := q.ToSql()
...
func (r *PRRepository) UpdateStatus(ctx context.Context, id string, status string) error {
	q := r.sb.Update("prs").Set("status", status).Where(sq.Eq{"id": id})
	sql, args, _ := q.ToSql()
	_, err := r.pool.Exec(ctx, sql, args...)
	return err
}
...
func (r *PRRepository) AddReviewer(ctx context.Context, rv *entity.PRReviewer) error {
	q := r.sb.Insert("pr_reviewers").Columns("pr_id", "reviewer_id", "assigned_from_team", "assigned_at").Values(rv.PRID, rv.ReviewerID, rv.AssignedFromTeam, rv.AssignedAt)
	sql, args, _ := q.ToSql()
	_, err := r.pool.Exec(ctx, sql, args...)
	if err != nil {
...
func (r *PRRepository) RemoveReviewer(ctx context.Context, prID, reviewerID string) error {
	q := r.sb.Delete("pr_reviewers").Where(sq.Eq{"pr_id": prID, "reviewer_id": reviewerID})
	sql, args, _ := q.ToSql()
	_, err := r.pool.Exec(ctx, sql, args...)
	if err != nil {
...
func (r *PRRepository) ListAssignedTo(ctx context.Context, userID string) ([]*entity.PR, error) {
	q := r.sb.Select("p.id", "p.title", "p.author_id", "p.status", "p.created_at", "p.updated_at").From("prs p").Join("pr_reviewers r ON p.id = r.pr_id").Where(sq.Eq{"r.reviewer_id": userID})
	sql, args, _ := q.ToSql()
...
func (r *PRRepository) ListReviewers(ctx context.Context, prID string) ([]entity.PRReviewer, error) {
	q := r.sb.Select("pr_id", "reviewer_id", "assigned_from_team", "assigned_at").From("pr_reviewers").Where(sq.Eq{"pr_id": prID})
	sql, args, _ := q.ToSql()
	rows, err := r.pool.Query(ctx, sql, args...)
	if err != nil {
		return nil, err
